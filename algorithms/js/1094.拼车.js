...
// @lc code=start
/**
 * 先按照上车顺序排序, 每次从上车队列头取出一组, 判断是否能上车, 直至上车队列为空
 * @param {number[][]} trips
 * @param {number} capacity
 * @return {boolean}
 */
var carPooling = function(trips, capacity) {
  trips.sort(([, start1, end1], [, start2, end2]) =>
    start1 !== start2 ? start1 - start2 : end1 - end2
  );
  let car = [];
  while (trips.length) {
    const tmp = trips.shift();
    car = car.filter(([, , end]) => end > tmp[1]);
    const cur = car.reduce((acc, a, b) => ((acc += a[0]), acc), 0);
    if (tmp[0] + cur > capacity) {
      return false;
    }
    car.push(tmp);
  }
