...
StockSpanner.prototype.next = function(price) {
  this.index++;
  let count = 1;
  const index = this.stack.findIndex(v => v <= price);
  if (~index) {
    this.stack[index] = price;
    this.orders[index] = this.index;
    this.orders.length = index + 1;
    this.stack.length = index + 1;
    count = index === 0 ? this.index + 1 : this.index - this.orders[index - 1];
  } else {
    this.stack.push(price);
    this.orders.push(this.index);
  }
  return count;
};
/**
